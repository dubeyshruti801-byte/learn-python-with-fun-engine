...
def validate_code(code, questions):
    ...
    def fake_input(prompt=""): nonlocal input_index # If only one input() is used (split-style question) if expects.get("min_value") and expects.get("requires_input"): if input_index == 0: input_index = len(input_buffer) return " ".join(input_buffer) # Otherwise normal sequential input if input_index < len(input_buffer): value = input_buffer[input_index] input_index += 1 return value return ""
    def fake_input(prompt=""):
      nonlocal input_index

      if expects.get("input_style") == "split":
        if input_index == 0:
