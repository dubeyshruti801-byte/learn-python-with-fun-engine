...
def validate_code(code, questions):
    ...
    for node in ast.walk(tree):
        ...
        #Track collection
        if isinstance(node, (ast.List, ast.Tuple, ast.Dict, ast.Set, ast.ListComp)):
           has_collection= True
   ...
   #----------Collection Validation---------#
    if expects.get("requires_collection"):
       if not has_collection:
          return False, "Collection (list) not used ❌"
    ...
    if expects.get("variable_flow"):
       min_vars= expects.get("min_variable", 1)
       if len(variables_used) < min_vars:
          return False, "Not enough variable used ❌"
