...
try:
    student_output = ast.literal_eval(raw_output)
except:
    student_output = raw_output

expected_raw = test_case["expected_output"][0]
...
if student_output == expected_output:
    ...
    print(f"Output mismatch ❌ Expected: {expected_output}, Got: {student_output}")
...
def validate_code(code, questions):
    ...
    for case in test_cases:
       ...
       if expects.get("collection_type") in ["set" , "dict" , "tuple"]:
         normalized_expected = normalize_output(expected_raw[0])
       else:
