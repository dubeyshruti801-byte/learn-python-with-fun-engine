...
def validate_code(code, questions):
    ...
    # If only one input() is used (split-style question)
      if expects.get("min_value") and expects.get("requires_input"):
        if input_index == 0:
            input_index = len(input_buffer)
            return " ".join(input_buffer)

    # Otherwise normal sequential input
    ...
    for case in test_cases:
       ...
       actual_raw = "\n".join(output).strip()
       expected_raw = case.get("expected_output")[0]
