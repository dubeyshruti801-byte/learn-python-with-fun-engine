...
def validate_code(code, questions):
    ...
    for case in test_cases:
       ...
       input_buffer= [x.strip() for x in raw_input.replace(",", " ").split()]
       for case in test_cases:
        output.clear()
        input_index = 0
    ...
    if "expected_output" in case:
        expected = case["expected_output"].replace(",", " ").strip()
        actual = " ".join(output).replace(",", " ").strip()

        if actual != expected:
            return False, f"Output mismatch ❌ Expected: {expected}, Got: {actual}"
